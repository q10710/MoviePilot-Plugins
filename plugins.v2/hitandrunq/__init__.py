...
from .entities import HNRStatus, TaskType, TorrentHistory, TorrentTask
from .helper import FormatHelper, TimeHelper, TorrentHelper
from .hnrconfig import HNRConfig, NotifyMode, SiteConfig
...
class DownloaderContext:
    """单个下载器的运行上下文（服务信息、实例与种子辅助器）。"""

    def __init__(self, name: str, service_info: ServiceInfo, torrent_helper: TorrentHelper):
        """初始化下载器上下文。"""
        self.name = name
    ...
    @property
    def dl_type(self) -> Optional[str]:
        """下载器类型：qbittorrent / transmission。"""
        return self.torrent_helper.dl_type

    @property
    def is_qbittorrent(self) -> bool:
        """是否为 qBittorrent 下载器（标签能力仅 qb 支持）。"""
        return self.dl_type == "qbittorrent"


class HitAndRunQ(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "hitandrunq_"
    # 加载顺序
    plugin_order = 24
    # 可使用的用户级别
    auth_level = 2
    ...
    def init_plugin(self, config: dict = None):
        ...
        # 多下载器模式下按需构建下载器上下文，见 __get_context / __get_contexts
        self.torrent_helper = None
    ...
    def __get_context(self, name: str) -> Optional[DownloaderContext]:
        """
        获取指定下载器的运行上下文（服务信息、实例与种子辅助器）。
        """
        if not name:
            return None

        service = self.downloader_helper.get_service(name=name)
        if not service or not getattr(service, "instance", None):
            logger.warning(f"获取下载器实例失败：{name}")
            return None
        ...
        return DownloaderContext(name=name,
                                 service_info=service,
                                 torrent_helper=TorrentHelper(service.instance))

    def __get_contexts(self) -> Dict[str, DownloaderContext]:
        """
        获取所有已配置且可用的下载器上下文。
        """
        ...
        if not self._hnr_config:
            return contexts
        for name in self._hnr_config.get_downloaders():
            context = self.__get_context(name)
            if context:
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        """
        # 多下载器支持：qBittorrent 与 Transmission 均可参与监控
        downloader_options = [{"title": config.name, "value": config.name}
                              for config in self.downloader_helper.get_configs().values()
                              if config.type in ("qbittorrent", "transmission")]
        return [
            {
                'component': 'VForm',
                'content': [
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            ...
                                            'chips': True,
                                            'clearable': True,
                                            'hint': '可同时监控多个下载器；qb→tr 等转移做种会被识别为“已转移”，不再误报种子被删除',
                                            'persistent-hint': True
                                        }
                                    }
                                ]
    ...
    def check(self):
        """
        检查服务：遍历所有已配置的下载器，同步种子状态、识别转移做种并更新H&R状态
        """
        if not self._hnr_config or not self._hnr_config.get_downloaders():
            logger.warning("尚未配置下载器，跳过本轮检查")
            return
        ...
        with lock:
            ...
            if not contexts:
                logger.warning("没有可用的下载器，本轮检查跳过（不做任何删除判定）")
                return

            # 逐个下载器获取种子：hash -> 所在下载器、各下载器种子明细
            presence: Dict[str, str] = {}
            seeding_by_downloader: Dict[str, Dict[str, Any]] = {}
            torrents_by_downloader: Dict[str, Dict[str, Any]] = {}
            unavailable_downloaders = set()

            for name, context in contexts.items():
                seeding_torrents = context.torrent_helper.get_torrents()
                if seeding_torrents is None:
                    # 下载器本轮不可用：跳过，且该下载器上的种子不参与“种子消失”判定
                    unavailable_downloaders.add(name)
                    logger.warning(f"下载器 {name} 获取种子失败，本轮跳过该下载器的删除判定")
                    continue
                seeding_dict = {
                    context.torrent_helper.get_torrent_hashes(torrents=torrent): torrent
            ...
            for name, context in contexts.items():
                if name not in seeding_by_downloader:
                    continue
                self.__update_seeding_tasks_based_on_tags(context=context,
            ...
            self.__update_torrent_tasks_state(torrent_tasks=torrent_tasks,
                                              presence=presence,
                                              torrents_by_downloader=torrents_by_downloader,
                                              contexts=contexts)
    ...
    def __update_torrent_tasks_state(self, torrents: List[Any], torrent_tasks: Dict[str, TorrentTask]):
        ...
        for torrent_hash, torrent_task in torrent_tasks.items():
            ...
            uploaded_offset = torrent_task.uploaded_offset or 0.0
            downloaded_offset = torrent_task.downloaded_offset or 0.0

            torrent_task.seeding_time = seeding_time_offset + (torrent_info.get("seeding_time", 0) or 0)
            torrent_task.uploaded = uploaded_offset + (torrent_info.get("uploaded", 0) or 0)
            torrent_task.downloaded = downloaded_offset + (torrent_info.get("downloaded", 0) or 0)
            if torrent_task.downloaded:
                torrent_task.ratio = torrent_task.uploaded / torrent_task.downloaded
            else:
                torrent_task.ratio = torrent_info.get("ratio", 0.0) or 0.0

    def __update_seeding_tasks_based_on_tags(self, context: DownloaderContext,
    ...
    def __sync_presence_and_transfer(self, torrent_tasks: Dict[str, TorrentTask],
                                     presence: Dict[str, str],
                                     unavailable_downloaders: set) -> None:
        """
        识别下载器之间的转移做种，并标记在所有可用下载器中都已消失的种子为已删除

        :param torrent_tasks: H&R 任务记录（hash -> TorrentTask）
        :param presence: 本轮所有可用下载器中的种子 hash -> 所在下载器名称
        :param unavailable_downloaders: 本轮获取失败的下载器，其上的种子不参与删除判定
        """
        ...
        for torrent_hash, torrent_task in torrent_tasks.items():
            ...
            current_downloader = presence.get(torrent_hash)
            if current_downloader:
                if not torrent_task.downloader:
                    # 历史记录缺少下载器信息：直接绑定，不做累计
                    torrent_task.downloader = current_downloader
                elif torrent_task.downloader != current_downloader:
                    # 同一 hash 出现在另一个下载器：判定为转移做种，累计此前的做种数据
            ...
            if torrent_task.downloader and torrent_task.downloader in unavailable_downloaders:
                logger.info(f"下载器 {torrent_task.downloader} 本轮不可用，"
                            f"暂不判定种子 {torrent_task.identifier} 是否被删除")
                continue
        ...
        if transferred_tasks:
            self.__log_and_send_torrent_task_update_message(
                title="【H&R种子任务已转移】",
                status="转移到其它下载器继续做种",
                reason="在其它下载器中找到相同种子的做种任务",
                torrent_tasks=transferred_tasks)

        for torrent_task in deleted_tasks:
    ...
    def __accumulate_transfer(self, torrent_task: TorrentTask, target_downloader: str) -> None:
        """
        种子转移到另一个下载器：把当前下载器已累计的做种数据并入偏移量，并更新所属下载器

        :param torrent_task: 发生转移的 H&R 任务
        :param target_downloader: 目标下载器名称
        """
        provided_seeding_time = max((torrent_task.seeding_time or 0) - (torrent_task.seeding_time_offset or 0), 0.0)
        provided_uploaded = max((torrent_task.uploaded or 0) - (torrent_task.uploaded_offset or 0), 0.0)
        provided_downloaded = max((torrent_task.downloaded or 0) - (torrent_task.downloaded_offset or 0), 0.0)

        torrent_task.seeding_time_offset = (torrent_task.seeding_time_offset or 0.0) + provided_seeding_time
    ...
    def handle_download_added_event(self, event: Event = None):
        ...
        event_context: Context = event.event_data.get("context")
        downloader = event.event_data.get("downloader")
        if not downloader:
            logger.info("触发添加下载事件，但没有获取到下载器信息，跳过后续处理")
            return

        if downloader not in self._hnr_config.get_downloaders():
            logger.info(f"触发添加下载事件，但没有监听下载器 {downloader}，跳过后续处理")
            return

        downloader_context = self.__get_context(downloader)
        if not downloader_context:
            logger.warning(f"下载器 {downloader} 不可用，跳过本次下载事件处理")
            return

        if not torrent_hash or not event_context or not event_context.torrent_info:
            logger.info("没有获取到有效的种子任务信息，跳过处理")
            return

        torrent_info = event_context.torrent_info
    ...
    def handle_brushflow_event(self, event: Event = None):
        ...
        downloader_context = self.__get_context(downloader)
        if not downloader_context:
            logger.warning(f"下载器 {downloader} 不可用，跳过本次刷流事件处理")
            return
        ...
        self.__process_event(torrent_hash, torrent_data, TaskType.BRUSH, context=downloader_context)
    ...
    def __process_event(self, torrent_hash: str, torrent_data: Union[dict, TorrentInfo], task_type: TaskType,
        ...
        with lock:
            ...
            # 处理种子任务
            self.__process_torrent_task(torrent_hash=torrent_hash, torrent_data=torrent_data,
        ...
        try:
            ...
            if add:
                if hnr_tag not in tags:
                    tags.append(hnr_tag)
                    context.torrent_helper.set_torrent_tag(torrent_hash=torrent_task.hash, tags=tags)
            else:
                if hnr_tag in tags:
                    tags.remove(hnr_tag)
                    context.torrent_helper.remove_torrent_tag(torrent_hash=torrent_task.hash, tags=[hnr_tag])
        except Exception as e:
    ...
    def __create_torrent_history(torrent_hash: str, torrent_data: Union[dict, TorrentInfo],
        ...
        """创建种子信息"""
        return HitAndRunQ.__create_torrent_instance(torrent_hash, torrent_data, TorrentHistory, task_type)
    ...
    def __convert_torrent_info_to_task(self, context: DownloaderContext, torrent: Any,
        ...
        if torrent_history:
            ...
            site_id, site_name = context.torrent_helper.get_site_by_torrent(torrent=torrent)
            if not site_name:
                return None
