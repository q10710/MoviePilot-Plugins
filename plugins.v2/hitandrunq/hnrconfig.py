...
class HNRConfig(BaseConfig):
    ...
    @classmethod
    def __empty_string_to_float(cls, v, info: ValidationInfo):
        """
        校验空字符
        """
    ...
    @field_validator("auto_cleanup_days", mode="before")
    @classmethod
    ...
    def __post_init__(self):
        ...
        初始化完成
        """
        self.__migrate_downloaders()
        self.__process_site_configs()

    def __migrate_downloaders(self):
        """
        兼容旧版单下载器配置：downloader 有值时自动迁移到 downloaders 列表。
        """
        if not self.downloaders and self.downloader:
            self.downloaders = [self.downloader]

    def get_downloaders(self) -> List[str]:
        """
        获取参与监控的下载器列表（兼容旧配置）。
        """
        downloaders = [item for item in (self.downloaders or []) if item]
        if not downloaders and self.downloader:
