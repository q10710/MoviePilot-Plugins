...
class SubscriptionCleanup:
    ...
    def handle_resource_download_history_clear(self, subscribe, context=None, episodes=None) -> bool:
        ...
        if not media_source or not media_id or self._get_histories is None:
            return True
        ...
        if media_type == MediaType.TV:
            ...
            if episode_scoped_cleanup and not target_episodes:
                ...
                return True
        histories = self._get_histories(media_source, media_id, subscribe.type, season) or []
    ...
    def migrate_snapshot_identities(self) -> int:
        ...
        迁移在同一个持久化 key 的原子读改写中完成。无法转换的记录保持原样，避免在来源
        不明确时猜测身份；底层保存失败时异常向上传递，旧数据也不会被提前删除。
        """
        if not self._update:
            return 0
        migrated = {"count": 0}
        ...
        def updater(data: dict) -> dict:
            for task_key, task in list((data or {}).items()):
                if not isinstance(task, dict):
                    continue
                if task.get("media_source") and task.get("media_id"):
                    continue
                tmdb_id = task.get("tmdbid")
                if tmdb_id in (None, ""):
                    continue
                converted = dict(task)
                converted["media_source"] = "themoviedb"
                converted["media_id"] = str(tmdb_id)
                converted.pop("tmdbid", None)
                data[task_key] = converted
                migrated["count"] += 1
            return data
    ...
    def handle_history_clear(self, event) -> bool:
        ...
        media_source = getattr(mediainfo, "media_source", None) if mediainfo else None
        media_id = getattr(mediainfo, "media_id", None) if mediainfo else None
        if not media_source or not media_id or not self._update:
            return False
        clear_task = self._claim_clear_history_task(str(media_source), str(media_id), data)
        if not clear_task:
            return False
        try:
    ...
    def _claim_clear_history_task(self, media_source: str, media_id: str, event_data) -> Optional[dict]:
        """原子占用本次整理要清理的快照记录，避免并发 TransferIntercept 重复删除。"""
        claimed = {}

        def updater(data: dict) -> dict:
            task_key, task, event_episodes = self._match_clear_history_task(
                media_source, media_id, event_data, data,
            )
            if not task_key or not task:
                return data
            if self._clear_history_task_expired(task):
                detail(f"订阅整理拦截：{media_source}:{media_id} 的清理事务已超过 36 小时，丢弃且不删除媒体库文件")
                return data
    ...
    def _match_clear_history_task(self, media_source: str, media_id: str, event_data,
                                  snapshots: dict) -> tuple[Optional[str], Optional[dict], list[int]]:
        """按规范媒体身份和整理目标集匹配待消费清理事务。"""
        ...
        for key, task in (snapshots or {}).items():
            if (
                str((task or {}).get("media_source") or "") != media_source
                or str((task or {}).get("media_id") or "") != media_id
            ):
                continue
    ...
    @classmethod
    def _task_key(cls, media_source, media_id, media_type: MediaType, season: Optional[str],
                  scene: str, target_episodes: Optional[list[int]]) -> str:
        """生成订阅清理事务键；同一媒体的不同场景和集范围必须互不覆盖。"""
