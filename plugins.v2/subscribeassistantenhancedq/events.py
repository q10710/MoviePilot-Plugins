...
class EventProxy:
    ...
    @staticmethod
    def _supports_subscribe(subscribe) -> bool:
        """仅电影和电视剧订阅进入增强生命周期与资源处理链路。"""
        return resolve_subscribe_media_type(subscribe) in (MediaType.MOVIE, MediaType.TV)
    ...
    def _format_episodes_refresh_label(data: SubscribeEpisodesRefreshEventData) -> str | None:
        ...
        if media_source and media_id:
            markers.append(f"media={media_source}:{media_id}")
        scene = data.scene
        if scene:
    ...
    def on_completion_check(self, event):
        ...
        if data is None or not self._supports_subscribe(data.subscribe):
            return
        guard.handle(event)
    ...
    def on_episodes_refresh(self, event):
        ...
        if subscribe is not None and not self._supports_subscribe(subscribe):
            return
        label = None
    ...
    def on_subscribe_added(self, event):
        ...
        subscribe_oper = self.get("subscribe_oper")
        subscribe = subscribe_oper.get(subscribe_id) if subscribe_oper else None
        if not subscribe or not self._supports_subscribe(subscribe):
            return
    ...
    def on_resource_selection(self, event):
        ...
        if not contexts:
            return
        subscribe_oper = self.get("subscribe_oper")
        _, subscribe = subscribe_from_source(getattr(data, "origin", None), subscribe_oper)
        if subscribe is not None and not self._supports_subscribe(subscribe):
            return
