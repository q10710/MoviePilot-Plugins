...
class PluginConfig:
    ...
    @property
    def relocate_enabled(self) -> bool:
        """订阅超时收容：下载超时仍未完成的订阅种子移入收容目录保留做种，而不是直接删除。"""
    ...
    def default_hr_hours(self) -> float:
        ...
        return value if value > 0 else 168.0

    @property
    def site_hr_hours(self) -> str:
        """站点 H&R 时长配置文本，格式：站点名:小时，多个用逗号或换行分隔。"""
    ...
    def full_hr_sites(self) -> str:
        ...
        仅用于「该站解析不出种子级 H&R 标记」时的站点级兜底；
        正常情况收容判定以本插件下载当时存档的 TorrentInfo.hit_and_run 为准。
