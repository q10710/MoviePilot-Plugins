...
LABELS = {
    ...
    "hr_mode": "超时处理模式",
    "hr_auto_scan": "自动刷新站点时长",
    "relocate_enabled": "超时收容（改为收容不删种）",
    "relocate_dir": "收容目录",
    "relocate_after_hours": "收容门槛（小时）",
    "relocate_delete_files": "到期删除源文件",
    "relocate_incomplete_days": "未完成清理天数",
    "default_hr_hours": "默认H&R时长（小时）",
    "site_hr_hours": "站点H&R时长（可选）",
...
HINTS = {
    ...
    "reset_task": "保存后将重置所有待定/暂停/监控等任务数据，执行后自动复位",
    ...
    # 订阅清理
    "download_monitor_enabled": "下载任务检查总开关：H&R 种子超时改为收容保种，普通种子按低进度超时删除",
    ...
    "hr_mode": "二选一：收容=H&R种子超时转入收容目录保种（此时排除标签不生效）；排除=带排除标签的种子跳过处理",
    "hr_auto_scan": "每天自动抓取各站 H&R 页面并合并进「站点H&R时长」（新值更小时保留旧值，避免提前删除）",
    "relocate_enabled": "开启后，仅 H&R 种子的超时会改为移入收容目录保种；普通种子仍按原逻辑删除",
    "relocate_dir": "收容种子移动到这里，该目录不参与媒体库整理",
    "relocate_after_hours": "H&R 种子下载超过该时长仍未完成即移入收容目录；留空则自动跟随「下载超时时间」（即「下载超时时间(分钟)/60」）",
    "relocate_delete_files": "关闭则到期仅删除下载任务、保留已下载文件",
    "relocate_incomplete_days": "收容后仍未下载完成满 N 天，按「站点不计 H&R」删除任务与文件；0 表示不清理",
    "default_hr_hours": "站点页面与种子信息都取不到H&R时长时使用的默认值，默认168小时",
    "site_hr_hours": "格式：站点名:小时，多个用逗号或换行分隔，例如 CARPT:168,听听歌:72；这里填了时长的站点即视为 H&R 站点",
...
TABS = [
    ("订阅清理", [
        ...
        [("default_tracker_response", 12)],
        ["relocate_enabled", "relocate_after_hours", "relocate_delete_files"],
        [("relocate_incomplete_days", 4)],
        ["hr_mode"],
        ["hr_auto_scan"],
        [("relocate_dir", 6), ("default_hr_hours", 6)],
        [("site_hr_hours", 12)],
    ]),
    ("订阅待定", [
